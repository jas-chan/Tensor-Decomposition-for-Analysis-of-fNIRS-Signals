...
%activations on the left and right hemisphere. Taking condition indeces to 
% use in bar plots
   
    function CPD_plots(U,r_components,left,condition_ind,conditions,saveDir,tick_time,tick_freq)
    ...
    for i=1:r_components
        ...
        for conditionI=1:length(conditions)
            ...
            bar(starti:endi,U{4}(starti:endi,i));
            starti=endi+1;
        end
        ...
        if left
            imwrite(F.cdata, fullfile(saveDir,'figures',...
                strcat('left',num2str(i),'.png')), 'png')
            close 
        else
            imwrite(F.cdata, fullfile(saveDir,'figures',...
                strcat('right',num2str(i),'.png')), 'png')
            close 
        end
    end
